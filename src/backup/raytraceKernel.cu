...
__global__ void rayTracerIterative(ray thisRay, int index, int time, int rayDepth, glm::vec3* colors,
                            staticGeom* geoms, int numberOfGeoms, material* materials, int numberOfMaterials, int* lightSources, int numberOfLights){
...
//  dim3 fullBlocksPerGrid((int)ceil(float(renderCam->resolution.x)/float(tileSize)), (int)ceil(float(renderCam->resolution.y)/float(tileSize)));
  ...
  // TODO: construct initial ray pool
  ...
  glm::vec3* cudaray = NULL;
  cudaMalloc((void**)&cudaray, rayPoolCount*sizeof(ray));
  cudaMemcpy( cudaray, rayPool, rayPoolCount*sizeof(ray), cudaMemcpyHostToDevice);

  rayTracerIterativePrimary<<<blocksPerGrid, threadsPerBlock>>>(renderCam->resolution, iterations, cam, rayPool);
